...
Each node calls Gemini 2.5 Flash for domain-specific reasoning,
...
def _query_gemini(messages: list, label: str = "") -> str:
    """
    Call Gemini 2.5 Flash with OpenAI-format messages.
    Prints progress so it appears in the console log stream.
    """
    from config import gemini_llm
    if not gemini_llm:
        raise ValueError("Gemini not configured. Check GOOGLE_API_KEY in .env")

    prefix = f"[LangGraph{' · ' + label if label else ''}]"
    print(f"  {prefix} querying Gemini 2.5 Flash…")
    ...
    print(f"  {prefix} ✓ Gemini responded")
    return reply
...
def plan_node(state: BankingAnalyticsState) -> BankingAnalyticsState:
    """
    Planning node – Gemini creates a structured analysis plan covering
    data needs, ML tasks, and visualization requirements.
    """
