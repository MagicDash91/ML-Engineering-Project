...
The final executive report is drafted by the Manager agent then enriched by **Gemini 2.5 Flash post-processing** with concrete KPIs, responsible teams, 30/60/90-day milestones, and segment-specific retention strategies.
...
Phase 3 — Gemini Post-Processing (app.py)
     ...
     │   → Prioritised Next Steps with owners, deadlines, and success metrics
     │
     ▼
Browser UI — 5 Tabs
     ├── Console    — real-time SSE agent log stream
     ├── Report     — blog-style: executive summary + per-chart sections
...
```
Bank_Agent/
├── app.py                  # FastAPI backend + SSE streaming + chat endpoints + security layers
...
│
├── static/
│   └── index.html          # Bootstrap browser UI (5 tabs + Conversation security UI)
│
└── outputs/
...
[Post-processing]  Gemini 2.5 Flash enhances report → KPIs, teams,
        ...
        │
        ▼
[Browser]  Report tab  → Executive summary + per-chart sections
