...
import matplotlib.dates as mdates   # required for DateFormatter in generate_dashboard
...
def generate_visualization(
    ...
    Args:
        table_name:  PostgreSQL source table (string). Example: 'churn' or 'churn_encoded'.
        chart_type:  Chart type (string). One of: 'bar', 'line', 'scatter', 'histogram',
                     'histplot', 'heatmap', 'pie'. Use 'histplot' to compare a numeric
                     column split by a categorical group. Use 'heatmap' only with
                     table='churn_encoded'.
        x_column:    Column for X-axis or grouping column for pie/heatmap (string).
        y_column:    Column for Y-axis; pass empty string '' for histogram/heatmap (string).
        title:       Chart title; pass empty string '' to auto-generate (string).
        hue_column:  Column for colour grouping; pass empty string '' if not needed (string).

    Returns:
        JSON string with chart_path, gemini_vision_analysis, and nvidia_recommendations.
    """
    try:
        ...
        try:
            nvidia_recs = query_nvidia([
                ...
                {"role": "user",   "content": (
                    ...
                    f"Gemini vision analysis:\n{gemini_analysis}\n\n"
                    "Based on the full analysis above, give exactly 3 concise actionable "
                    "recommendations in 1 sentence each."
                )},
            ], max_tokens=512)
        except Exception as _nv_err:
            print(f"[generate_visualization] NVIDIA recs failed: {_nv_err}")
            nvidia_recs = ""

        # ── Persist metadata so the blog report can pair image + analysis ──
        ...
        try:
            ...
            print(f"[session_charts] saved entry #{len(_existing)}: {auto_title}")
        except Exception as _meta_err:
            print(f"[session_charts] metadata write failed: {_meta_err}")
...
def generate_dashboard(table_name: str, analysis_focus: str = "general") -> str:
    ...
    try:
        ...
        try:
            ...
            with open(_meta_path, "w", encoding="utf-8") as _fh:
                json.dump(_existing, _fh, indent=2, ensure_ascii=False)
            print(f"[session_charts] saved dashboard entry #{len(_existing)}")
        except Exception as _meta_err:
            print(f"[session_charts] dashboard metadata write failed: {_meta_err}")
