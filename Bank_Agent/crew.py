...
data team:
    ...
    Manager (CDO)    – synthesises all outputs into the executive briefing (NVIDIA LLaMA)
    ...
    Data Analyst     – tool selection / SQL / coding (NVIDIA LLaMA)
                       chart & dashboard vision analysis (Gemini 2.5 Flash, inside tools)
...
def create_manager_agent() -> Agent:
    return Agent(
        ...
        llm=nvidia_llm,
        allow_delegation=False,   # sequential process: synthesise directly, don't re-delegate
        verbose=True,
        max_iter=15,              # raised from 8 — synthesis of 4 task contexts needs headroom
    )
...
def create_data_analyst_agent() -> Agent:
    return Agent(
        ...
        llm=nvidia_llm,               # NVIDIA LLaMA: tool selection, SQL, coding, PDF/PPT logic
                                      # Gemini 2.5 Flash: called inside generate_visualization /
                                      # generate_dashboard tools for vision-based chart analysis
        tools=[
            generate_visualization,
            generate_dashboard,
            generate_text_report,
        ...
        allow_delegation=False,
        verbose=True,
        max_iter=30,         # needs 13+ tool calls; raised from 20 to prevent early stop before PDF/PPT
        max_retry_limit=3,
    )
...
def create_tasks(
    ...
    task_analyst = Task(
        description=(
            ...
            f"Guidelines:\n"
            f"- Only create charts that add genuine business value.\n"
            f"- For heatmap use table='churn_encoded'. For all other types use table='churn'.\n"
            f"- Gemini vision analysis is applied automatically to every chart.\n\n"
            f"HOW TO CALL generate_data_visualization — pass these exact string arguments:\n"
            f"  table_name  : 'churn' or 'churn_encoded'\n"
            f"  chart_type  : 'pie', 'bar', 'histogram', 'histplot', 'scatter', or 'heatmap'\n"
            f"  x_column    : column name string (e.g. 'Churn', 'tenure', 'Contract')\n"
            f"  y_column    : column name string, or '' if not needed\n"
            f"  title       : descriptive title string, or ''\n"
            f"  hue_column  : column name string for colour grouping, or ''\n\n"
            f"Examples:\n"
            f"  generate_data_visualization('churn', 'pie', 'Churn', '', 'Churn Distribution', '')\n"
            f"  generate_data_visualization('churn', 'histogram', 'tenure', '', '', '')\n"
            f"  generate_data_visualization('churn', 'histplot', 'MonthlyCharges', '', '', 'Churn')\n"
            f"  generate_data_visualization('churn', 'scatter', 'tenure', 'MonthlyCharges', '', 'Churn')\n"
            f"  generate_data_visualization('churn', 'bar', 'Contract', 'MonthlyCharges', '', '')\n"
            f"  generate_data_visualization('churn_encoded', 'heatmap', 'Churn', '', '', '')\n\n"
            f"Required deliverables (always produce these):\n"
            f"1. Individual charts — at least one per insight category from the ML results.\n"
            f"   Only create charts that add genuine business value; do NOT generate a dashboard.\n"
            f"2. A text report: generate_text_report with ALL team analysis results\n"
            f"3. A PDF report: generate_pdf_report with all chart paths\n"
            f"4. A PowerPoint: generate_ppt_report with all chart paths"
        ),
        expected_output=(
    ...
    task_manager = Task(
        description=(
            ...
            f"   Do NOT include file paths, folder locations, or system paths of any kind."
        ),
        expected_output=(
...
# ============================================================
# Phase 2 only — called by app.py after HITL approval
# ============================================================

def run_crewai_phase(analysis_request: str, langgraph_plan: dict = None) -> dict:
    """
    Run Phase 2: CrewAI multi-agent execution.
    Called directly by app.py (after HITL approval) or by
    run_banking_analysis_crew (legacy / no-HITL path).
    """
...
# ============================================================
# Legacy entry point (no HITL — both phases in one call)
# ============================================================
...
def run_banking_analysis_crew(
    ...
    # ── Phase 2: CrewAI ───────────────────────────────────────────
    phase2 = run_crewai_phase(analysis_request, langgraph_plan)
    results["crew_output"] = phase2["crew_output"]
    results["status"]      = "completed"
