...
def _gemini(prompt: str, system: str = "", _timeout: int = 130) -> str:
    ...
    def _call() -> str:
        ...
        for attempt in range(3):
            try:
                ...
                last_exc = exc
                if attempt < 2:
                    print(f"[ContentTools] Ollama error (attempt {attempt+1}/3): {exc} — retrying in 5s", flush=True)
                    time.sleep(5)
        print(f"[ContentTools] Ollama failed after 3 attempts: {last_exc}", flush=True)
    ...
    try:
        ...
        ex.shutdown(wait=False)
        print(f"[ContentTools] Ollama hard timeout ({_timeout}s) – returning fallback", flush=True)
    ...
    try:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as ex:
            ...
            try:
                ...
                return "[Poster generation timed out after 120s — retry later.]"
    except Exception as exc:
...
def generate_content_report(summary: str, brand_name: str) -> str:
    ...
    report = _gemini(
        prompt=(
            ...
            "## Performance Projections, ## Next Steps."
        ),
