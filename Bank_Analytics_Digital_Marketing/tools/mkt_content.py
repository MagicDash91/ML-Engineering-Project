"""
tools/mkt_content.py – Marketing content creation tools including Veo 3 video generation.
Mirrors Digital_Marketing_Agent/tools/content_tools.py with paths adjusted.
"""
...
def generate_video_content(
    ...
    try:
        ...
        if not saved and hasattr(video_data, "uri") and video_data.uri:
            ...
            uri = video_data.uri
            uri = f"{uri}?key={GOOGLE_API_KEY}" if "?" not in uri else f"{uri}&key={GOOGLE_API_KEY}"
