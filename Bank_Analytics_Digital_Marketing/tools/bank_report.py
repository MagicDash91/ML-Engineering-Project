"""
tools/bank_report.py – Professional PDF and PowerPoint report generation for banking analytics.
Mirrors Bank_Agent/tools/report_tools.py with paths adjusted to this system.
"""
...
def generate_pdf_report(
    ...
    Args:
        report_content: Markdown-formatted report text.
    ...
    try:
        ...
        s_title = ParagraphStyle("s_title", parent=base["Title"], fontSize=22, textColor=BLUE,
                                  spaceAfter=18, alignment=TA_CENTER, fontName="Helvetica-Bold")
        s_h1    = ParagraphStyle("s_h1", parent=base["Heading1"], fontSize=14, textColor=BLUE,
                                  spaceBefore=16, spaceAfter=8, fontName="Helvetica-Bold")
        s_h2    = ParagraphStyle("s_h2", parent=base["Heading2"], fontSize=12, textColor=LIGHT_BLUE,
        ...
        return json.dumps({"status": "success", "pdf_path": pdf_path,
                           "charts_included": len([c for c in chart_list if os.path.exists(c)])}, indent=2)
...
def generate_ppt_report(
    ...
    try:
        ...
        C_PRIMARY = RGBColor(0x15, 0x65, 0xC0)
        ...
        def add_chart_slide(chart_path, label):
            ...
            if os.path.exists(chart_path):
                slide.shapes.add_picture(chart_path, Inches(0.5), Inches(1.1), Inches(12.33), Inches(6.0))

        add_cover(report_title, f"Banking Analytics Report  •  {datetime.now().strftime('%B %Y')}")
        ...
        return json.dumps({"status": "success", "ppt_path": ppt_path,
                           "slides_created": len(prs.slides)}, indent=2)
    except Exception as e:
        return json.dumps({"status": "error", "message": str(e)})
