...
# Serve HTML Page
# Serve HTML Page with Empty Data
@app.get("/", response_class=HTMLResponse)
def serve_homepage():
    template = Template(html_template)
    return template.render(data=None)  # Explicitly set `data=None`
...
def fetch_churn_data(
    ...
    """Fetch churn data filtered by transaction_date and render it in HTML"""
    conn = create_connection()
    if conn:
        try:
            query = "SELECT * FROM public.churn"  # Default query

            # Apply date filtering if parameters are provided
            if start_date and end_date:
                query += f" WHERE transaction_date BETWEEN '{start_date}' AND '{end_date}'"
            elif start_date:
                query += f" WHERE transaction_date >= '{start_date}'"
            elif end_date:
                query += f" WHERE transaction_date <= '{end_date}'"
